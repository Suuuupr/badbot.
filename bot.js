...
client.on("ready", async  => {
setInterval(function(){
client.channels.find('id', '525489977962790922').setName("💔🌹{W");
client.channels.find('id', '525489977962790922').setName("💔🌹{We");
client.channels.find('id', '525489977962790922').setName("💔🌹{Wel");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welc");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welco");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcom");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcome");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcome T");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcome To");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcome To K");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcome To KI");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcome To KIN");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcome To KING");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcome To KINGT");
client.channels.find('id', '525489977962790922').setName("💔🌹{Welcome To KINGTOP}💔🌹");
  }, 60000);
});
...
client.on('message', message => {
  ...
  ** __Bid__** مـعـلومأت عـنـك
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bbot__** مـعـلومـات الـبـوت
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bping__** سـرعـة اتـصـال الـبـوت
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bserver__** مـعـلومأت الـسـيـرفـر
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bdate__** الـتـاريـخ
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bmembers__** حـألـة الاعـضـاء
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Binv__** دعـوة الـبـوت لـسـيرفـرك
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Brules__** كود القوانين
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bsupport__** لاظهار سيرفر السبورت
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bccolors__** لانشاء اللوان حسب الرقم
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **قريبأ**
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **قريبأ**
  ...
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bangaz__** انـجـأزات مـايـن كـرافـت
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
   **__Bmarry__** الـزواج
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Brps__** حـجـر ورق مـقـص
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bعقاب__**  
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bهل تعلم__**
   ...
   client.on("message", message => {
      if (message.content === "Bhelp") {
       const embed = new Discord.RichEmbed() 
           .setColor("#57FEFF")
           .setDescription(`**⚙️ اوامر   ⚙️**
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bban__** حـظـر الـعـضـو مـن الـسـيـرفـر
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bkick__** طـرد الـعـضـو مـن الـسـيـرفـر
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bbc__** بـرودكـأسـت
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bmute__** مـيوت
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bunmute__** فـك الـمـيوت
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bclear__** مـسـح الـشـأت
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bcl__** تـقـفـيل الـشـات
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bop__** فـتـح الـشـات
  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bserver__** سيرفر
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bsetwlc__** كود السيت ولكم
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  **__Bsetvoice__** الفويس اون لاين
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
لدعوةالبوت Binv`)
     message.author.sendEmbed(embed)
     }
